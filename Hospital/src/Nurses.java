...
public class Nurses extends JFrame {
   ...
   private JTextField chart_nnum_tf;
   ...
   public Nurses() {
      ...
      pname_lb.setBounds(37, 47, 85, 53);
      panel.add(pname_lb);
      
      pname_tf = new JTextField();
      pname_tf.setBounds(88, 47, 316, 47);
      panel.add(pname_tf);
      ...
      chart_textArea.setFont(new Font("맑은 고딕 Semilight", Font.PLAIN, 18));
      chart_textArea.setText("차트 내용을 입력하세요");
      
      JLabel chart_pname = new JLabel("환자 주민번호");
      chart_pname.setFont(new Font("맑은 고딕", Font.PLAIN, 15));
      ...
      JLabel lblNewLabel_3_1 = new JLabel("작성 간호사 번호");
      lblNewLabel_3_1.setFont(new Font("맑은 고딕", Font.PLAIN, 15));
      lblNewLabel_3_1.setBounds(431, 29, 126, 47);
      panel_2.add(lblNewLabel_3_1);
      
      chart_nnum_tf = new JTextField();
      chart_nnum_tf.setColumns(10);
      chart_nnum_tf.setBounds(559, 37, 311, 38);
      panel_2.add(chart_nnum_tf);
      

      
      JButton ch_submit_btn = new JButton("저장");
      ch_submit_btn.setBounds(1248, 430, 95, 23);
      panel_2.add(ch_submit_btn);
      ch_submit_btn.addActionListener(new ActionListener(){
          public void actionPerformed(ActionEvent ae){
               try {

                 String query = "insert into chart values(null,'" + chart_textArea.getText() +"'," + dg_prrn_tf.getText()
                       + "," +chart_nnum_tf.getText()+ ")";

                 pstmt = SwingDemo.conn.prepareStatement(query);
                 int k = pstmt.executeUpdate(query);
                 System.out.println("pstmt:" + pstmt);
                 System.out.println("차트 추가 쿼리:" + query);
                 
                 if (k == 1) {
                    JOptionPane.showMessageDialog(null, "차트 내용이 등록 되었습니다.");
                    dg_prrn_tf.setText("");
                    chart_nnum_tf.setText("");
...
//	            System.out.println("rs:"+rs);
	         ...
	         while(rs.next()) {
	            ...
	            System.out.println("진료 쿼리 :"+e.getMessage());
	         }   
	      }
   public void selectChart() {
	      ...
	      try {
	            ...
	            model3.setNumRows(0);
	            String query = "SELECT c.char_num, c.chart_content, p.prrn, n.n_num\r\n"
	            		+ "FROM patient p\r\n"
	            		+ "JOIN chart c ON p.prrn = c.chart_prrn\r\n"
...
//	            System.out.println("rs:"+rs);
	         ...
	         while(rs.next()) {
	            ...
	            String p_num = rs.getString("prrn");
	            ...
	            System.out.println("차트 쿼리 :"+e.getMessage());
	         }   
	      }
